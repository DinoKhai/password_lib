...
type KeyValueSheetRow = {
  key?: unknown
  value?: unknown
}

type WorkbookSheet = object

type Workbook = {
  Sheets: Record<string, WorkbookSheet | undefined>
}
...
type XlsxUtils = {
  book_new(): Workbook
  book_append_sheet(workbook: Workbook, sheet: WorkbookSheet, name: string): void
  json_to_sheet(rows: Record<string, unknown>[]): WorkbookSheet
  sheet_to_json<T>(sheet: WorkbookSheet, options: SheetToJsonOptions): T[]
}

type XlsxGlobal = {
  utils: XlsxUtils
  write(workbook: Workbook, options: WriteOptions): ArrayBuffer | Uint8Array
  read(source: ArrayBuffer, options: ReadOptions): Workbook
}
...
const titleInput = getRequiredElement<HTMLInputElement>('title-input')
const usernameInput = getRequiredElement<HTMLInputElement>('username-input')
const passwordInput = getRequiredElement<HTMLInputElement>('password-input')
const notesInput = getRequiredElement<HTMLTextAreaElement>('notes-input')
const saveButton = getRequiredElement<HTMLButtonElement>('save-button')
const formTitle = getRequiredElement<HTMLElement>('form-title')
const customFieldsContainer = getRequiredElement<HTMLElement>('custom-fields')
const editorShell = getRequiredElement<HTMLElement>('editor-shell')
const notificationElement = getRequiredElement<HTMLElement>('notification')
const librarySummary = getRequiredElement<HTMLElement>('library-summary')
const themeToggleButton = getRequiredElement<HTMLButtonElement>('theme-toggle')
const themeToggleIcon = getRequiredElement<HTMLElement>('theme-toggle-icon')
const searchInput = getRequiredElement<HTMLInputElement>('search-input')
const entryList = getRequiredElement<HTMLElement>('entry-list')
const entryForm = getRequiredElement<HTMLFormElement>('entry-form')
const toggleDraftPasswordButton = getRequiredElement<HTMLButtonElement>('toggle-draft-password')
const draftEyeIcon = getRequiredElement<HTMLElement>('draft-eye-icon')
const addSectionButton = getRequiredElement<HTMLButtonElement>('add-section-button')
const clearButton = getRequiredElement<HTMLButtonElement>('clear-button')
const newEntryButton = getRequiredElement<HTMLButtonElement>('new-entry-button')
const exportButton = getRequiredElement<HTMLButtonElement>('export-button')
const importButton = getRequiredElement<HTMLButtonElement>('import-button')
const importInput = getRequiredElement<HTMLInputElement>('import-input')

let draft = createEmptyDraft()
let editingEntryId: string | null = null
let isEditorOpen = false
let draftPasswordVisible = false
let revealedPasswords: Record<string, boolean> = {}
    ...
    try {
      ...
      setNotification({
        ...
        message: getErrorMessage(
          error,
          'Import failed. Existing local data was kept unchanged.',
        ),
      })
      renderNotification()
    }
...
function createEmptyCustomField(): CustomField {
  return {
    id: createId('field'),
    label: '',
    value: '',
...
function loadInitialState(): AppState {
  try {
    ...
    setNotification({
      ...
      message: getErrorMessage(
        error,
        'Saved data could not be loaded. A fresh local library was created.',
      ),
    })
    return createInitialState()
  }
...
function setNotification(nextNotification: AppNotification | null): void {
...
function renderDraftPassword(): void {
  passwordInput.type = draftPasswordVisible ? 'text' : 'password'
  draftEyeIcon.innerHTML = getEyeIconMarkup(draftPasswordVisible)
  toggleDraftPasswordButton.setAttribute(
    'aria-label',
    draftPasswordVisible ? 'Hide password' : 'Show password',
  )
...
function saveEntry(): void {
  ...
  try {
    ...
    setNotification({
      ...
      message: getErrorMessage(
        error,
        'Unable to save this entry locally. Check browser storage availability.',
      ),
    })
    renderNotification()
  }
...
function buildEntryFromDraft(sourceDraft: EntryDraft, existingEntryId: string | null): PasswordEntry {
  const now = createTimestamp()
...
function startEditing(entryId: string): void {
...
function renderEntries(): void {
  ...
  if (appState.entries.length === 0) {
    entryList.append(
      createEmptyState(
        'No entries yet',
        'Create your first password record to start building your secure library.',
        'library',
      ),
    )
    return
  ...
  if (filteredEntries.length === 0) {
    entryList.append(
      createEmptyState(
        'No matches found',
        'Try a different title, username, or note keyword.',
        'search',
      ),
    )
    return
  ...
  filteredEntries.forEach((entry) => {
    ...
    if (isExpanded) {
      ...
      actions.append(
        createActionButton('Edit', 'edit', entry.id, 'ghost-button'),
        createActionButton('Delete', 'delete', entry.id, 'ghost-button danger'),
      )

      const details = document.createElement('dl')
      details.className = 'entry-details'
      details.append(
        createDetailBlock('Username', entry.username || '-'),
        createPasswordBlock(entry),
        createDetailBlock('Notes', entry.notes || '-'),
      )
...
function getFilteredEntries(): PasswordEntry[] {
...
function createDetailBlock(label: string, value: string): HTMLElement {
...
function createPasswordBlock(entry: PasswordEntry): HTMLElement {
  const wrapper = document.createElement('div')
  const title = document.createElement('dt')
  ...
  button.setAttribute(
    'aria-label',
    revealedPasswords[entry.id] ? 'Hide password' : 'Show password',
  )
...
function getEyeIconMarkup(visible: boolean): string {
  if (visible) {
    return `
...
function getChevronIconMarkup(expanded: boolean): string {
...
function getRowIconMarkup(): string {
  return `
    <svg viewBox="0 0 20 20" focusable="false">
...
function getEmptyLibraryIllustrationMarkup(): string {
  return `
...
function getEmptySearchIllustrationMarkup(): string {
  return `
    <svg viewBox="0 0 64 64" focusable="false">
...
function sortEntriesByUpdatedAt(entries: PasswordEntry[]): PasswordEntry[] {
...
function normalizeCustomField(value: unknown, usedIds: Set<string>): CustomField | null {
  if (!isRecord(value)) {
    return null
  }
...
function normalizeEntry(value: unknown, usedIds: Set<string>): PasswordEntry | null {
  if (!isRecord(value)) {
    return null
  }
...
function migrateState(value: unknown): AppState {
  ...
  if (version > SCHEMA_VERSION) {
    throw new Error(
      `This file uses schema version ${version}, which is newer than this app can read.`,
    )
  }
  ...
  return {
    schemaVersion: SCHEMA_VERSION,
    settings: {
      maskPasswordsByDefault: normalizeBoolean(
        rawSettings.maskPasswordsByDefault,
        fallbackState.settings.maskPasswordsByDefault,
      ),
      theme: normalizeTheme(rawSettings.theme, fallbackState.settings.theme),
    },
    entries: sortEntriesByUpdatedAt(
      rawEntries
    ...
    meta: {
      ...
      lastExportedAt:
        normalizeTrimmedString(rawMeta.lastExportedAt).length > 0
          ? normalizeTimestamp(rawMeta.lastExportedAt, fallbackState.meta.updatedAt)
          : null,
      lastImportedAt:
        normalizeTrimmedString(rawMeta.lastImportedAt).length > 0
...
function exportExcel(): void {
  ...
  const customFieldRows = appState.entries.flatMap((entry) =>
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(customFieldRows),
    CUSTOM_FIELDS_SHEET,
  )
  XLSX.utils.book_append_sheet(
    workbook,
  ...
  const safeBuffer = new ArrayBuffer(
    fileBytes instanceof Uint8Array ? fileBytes.byteLength : fileBytes.byteLength,
  )
  new Uint8Array(safeBuffer).set(
...
async function importExcel(file: File): Promise<AppState> {
  ...
  const entryRows = sheetToJson<EntrySheetRow>(workbook, ENTRIES_SHEET)
  const customFieldRows = sheetToJson<CustomFieldSheetRow>(workbook, CUSTOM_FIELDS_SHEET)
  const settingsRows = sheetToJson<KeyValueSheetRow>(workbook, SETTINGS_SHEET)
  const metaRows = sheetToJson<KeyValueSheetRow>(workbook, META_SHEET)

  const customFieldsByEntry = new Map<string, CustomFieldSheetRow[]>()
...
function sheetToJson<T>(workbook: Workbook, sheetName: string): T[] {
...
function rowsToMap(rows: KeyValueSheetRow[], sheetName: string): Record<string, string> {
